...
class Game {
    ...
    initializeBoard() {
        return Array.from({
            length: Game.SIZE
        }, ()=>Array(Game.SIZE).fill(0));
    }
    spawnTile() {
        const emptyTiles = [];
        for(let row = 0; row < Game.SIZE; row++){
            for(let col = 0; col < Game.SIZE; col++)if (this.state[row][col] === 0) emptyTiles.push({
                row,
                col
            });
        }
    ...
    canMove() {
        for(let row = 0; row < Game.SIZE; row++)for(let col = 0; col < Game.SIZE; col++){
            if (this.state[row][col] === 0 || row < Game.SIZE - 1 && this.state[row][col] === this.state[row + 1][col] || col < Game.SIZE - 1 && this.state[row][col] === this.state[row][col + 1]) return true;
        }
        return false;
    }
    move(direction) {
    ...
    compressRow(row) {
        ...
        return [
            ...filtered,
            ...Array(Game.SIZE - filtered.length).fill(0)
        ];
    }
    transpose(matrix) {
